...
$pdo = db();
$errors = [];
$selectedTestId = filter_var($_GET['test_id'] ?? $_POST['test_id'] ?? 0, FILTER_VALIDATE_INT) ?: 0;
$selectedSectionId = filter_var($_GET['section_id'] ?? $_POST['section_id'] ?? 0, FILTER_VALIDATE_INT) ?: 0;
...
$sectionStmt->execute([$selectedTestId ?: (int) ($tests[0]['id'] ?? 0)]);
...
if (is_post()) {
    ...
    if ($section) {
        ...
        $optionImages = [
            'a' => $newUploads['option_a_image_path'],
            'b' => $newUploads['option_b_image_path'],
            'c' => $newUploads['option_c_image_path'],
            'd' => $newUploads['option_d_image_path'],
        ];
        $errors = array_merge($errors, validate_question_language_fields($_POST, $sectionSlug, $optionImages));
    }

    if (!$errors) {
        $duplicate = $pdo->prepare('SELECT id FROM questions WHERE test_id = ? AND section_id = ? AND question_number = ? LIMIT 1');
        $duplicate->execute([$selectedTestId, $selectedSectionId, $questionNumber]);
        if ($duplicate->fetch()) {
    ...
    if (!$errors) {
        ...
        $insert->execute([
            ...
            $newUploads['question_image_path'],
            ...
            $newUploads['option_a_image_path'],
            $newUploads['option_b_image_path'],
            $newUploads['option_c_image_path'],
            $newUploads['option_d_image_path'],
            $correctOption,
            input_string('explanation_hi', 10000),
            input_string('explanation_en', 10000),
            $newUploads['explanation_image_path'],
...
admin_header('Add Question', $admin);
?>
<section class="panel">
    <div class="panel-header">
        <div>
            <h2>New Question</h2>
            <p>Add one bilingual question for a selected test section.</p>
        </div>
