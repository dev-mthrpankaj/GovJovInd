...
test('empty feed finishes loading with helpful next step',()=>{const {nodes:n}=setup([]);assert.equal(n.count.textContent,'0');assert.equal(n.select.disabled,true);assert.equal(n.empty.hidden,false);assert.match(n.empty.innerHTML,/quiz.html#examQuizFamilies/)});
...
test('topic dropdown lists all topics with counts',()=>{const {nodes:n}=setup([item('one','First'),item('two','Second','vocabulary')]);assert.match(n.select.innerHTML,/All Topics \(2\)/);assert.match(n.select.innerHTML,/grammar \(1\)/);assert.match(n.select.innerHTML,/vocabulary \(1\)/);assert.equal(n.select.disabled,false)});
