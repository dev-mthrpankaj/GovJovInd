...
(function(){
  ...
  const ACCOUNT_SYNC_URL=API_BASE+"session.php";
  const OTP_REQUEST_URL=API_BASE+"email-otp-request.php";
  const OTP_VERIFY_URL=API_BASE+"email-otp-verify.php";
  const RESET_OTP_REQUEST_URL=API_BASE+"password-reset-otp-request.php";
  const RESET_CONFIRM_URL=API_BASE+"password-reset-confirm.php";
  const $=s=>document.querySelector(s),isAndroidApp=/GovJobUpdatesApp/i.test(navigator.userAgent||"");
  let authStateKnown=false,pendingSignup=null,pendingReset=null;
  ...
  async function jsonRequest(url,{body={},token=""}={}){
    const headers={"Content-Type":"application/json"};if(token)headers.Authorization=`Bearer ${token}`;
    const response=await fetch(url,{method:"POST",headers,body:JSON.stringify(body),credentials:"omit",cache:"no-store"});
    let payload=null;try{payload=await response.json();}catch{}
    if(!response.ok||!payload?.success){const err=new Error(payload?.message||`Request failed (${response.status})`);err.code=payload?.error||"request_failed";throw err;}
    return payload;
  }
  ...
  async function syncMySqlAccount(user,extra={}){
    ...
    try{
      ...
      const payload=await jsonRequest(ACCOUNT_SYNC_URL,{token,body:{mobile:cleanMobile(extra.mobile||""),display_name:extra.display_name||user.displayName||""}});
    ...
    }catch(e){console.warn("[GovJobUpdates] MySQL account sync skipped; Firebase login remains active:",e.message);return null;}
  }
  ...
  async function completeVerifiedLogin(user,extra={},redirect=true){
    if(!isVerifiedUser(user))throw new Error("Email verification required.");
    rememberSession(user);syncHeader(user);markAuthReady();await syncMySqlAccount(user,extra);if(redirect)go(redirectTarget("dashboard.html"));
  }
  ...
  async function requestSignupOtp(user,extra={}){
    const token=await user.getIdToken(true);
    const payload=await jsonRequest(OTP_REQUEST_URL,{token,body:{mobile:cleanMobile(extra.mobile||""),display_name:extra.display_name||user.displayName||""}});
    return payload.challenge;
  }

  async function beginUnverifiedLogin(user,password){
    clearSession();syncHeader(null);
    const challenge=await requestSignupOtp(user,{});
    pendingSignup={email:user.email||"",password,name:user.displayName||"",mobile:"",challenge};
    window.GJUAuthOtpUI?.showSignupOtp(user.email||"");
    show("We sent a 6-digit verification code to your email.",false);
  }

  function bindOtpEvents(){
    window.addEventListener("gju:otp-verify-request",async event=>{
      const detail=event.detail||{};show("");
      try{
        if(detail.mode==="signup"){
          if(!auth.currentUser||!pendingSignup?.challenge)throw new Error("Verification session expired. Please login again.");
          const token=await auth.currentUser.getIdToken(true);
          await jsonRequest(OTP_VERIFY_URL,{token,body:{challenge:pendingSignup.challenge,code:detail.code}});
          await auth.currentUser.reload();
          await auth.currentUser.getIdToken(true);
          if(!auth.currentUser.emailVerified)throw new Error("Email verification could not be confirmed. Please try again.");
          const extra={mobile:pendingSignup.mobile||"",display_name:pendingSignup.name||auth.currentUser.displayName||""};
          pendingSignup=null;
          show("Email verified successfully. Opening your dashboard…",false);
          await completeVerifiedLogin(auth.currentUser,extra,true);
          return;
        }
        if(detail.mode==="reset"){
          if(!pendingReset?.challenge)throw new Error("Reset session expired. Please start again.");
          const payload=await jsonRequest(OTP_VERIFY_URL,{body:{challenge:pendingReset.challenge,code:detail.code,purpose:"password_reset"}});
          pendingReset.resetToken=payload.reset_token||"";
          if(!pendingReset.resetToken)throw new Error("Reset authorization was not created. Please try again.");
          window.GJUAuthOtpUI?.showResetPassword();
          show("OTP verified. Set your new password.",false);
        }
      }catch(err){show(readableError(err),true);}
    });

    window.addEventListener("gju:otp-resend-request",async event=>{
      const detail=event.detail||{};show("");
      try{
        if(detail.mode==="signup"){
          if(!auth.currentUser)throw new Error("Verification session expired. Please login again.");
          const challenge=await requestSignupOtp(auth.currentUser,{mobile:pendingSignup?.mobile||"",display_name:pendingSignup?.name||auth.currentUser.displayName||""});
          pendingSignup={...(pendingSignup||{}),email:auth.currentUser.email||detail.email||"",challenge};
        }else{
          const payload=await jsonRequest(RESET_OTP_REQUEST_URL,{body:{email:detail.email||pendingReset?.email||"",website:""}});
          pendingReset={email:detail.email||pendingReset?.email||"",challenge:payload.challenge||"",resetToken:""};
        }
        show("A new OTP has been sent.",false);
      }catch(err){show(readableError(err),true);}
    });

    window.addEventListener("gju:password-reset-otp-request",async event=>{
      const email=String(event.detail?.email||"").trim();show("");
      try{const payload=await jsonRequest(RESET_OTP_REQUEST_URL,{body:{email,website:""}});pendingReset={email,challenge:payload.challenge||"",resetToken:""};if(!pendingReset.challenge)throw new Error("Unable to start password reset.");window.GJUAuthOtpUI?.showResetOtp(email);show("If this email is registered, a 6-digit OTP has been sent.",false);}catch(err){show(readableError(err),true);}
    });

    window.addEventListener("gju:password-reset-submit",async event=>{
      try{
        if(!pendingReset?.resetToken)throw new Error("Reset authorization expired. Please verify OTP again.");
        await jsonRequest(RESET_CONFIRM_URL,{body:{reset_token:pendingReset.resetToken,password:event.detail?.password||""}});
        const email=pendingReset.email;pendingReset=null;window.GJUAuthOtpUI?.backToAuth();const field=$("#loginEmail");if(field)field.value=email;show("Password updated successfully. You can now login.",false);
      }catch(err){show(readableError(err),true);}
    });
  }
  ...
  function bindLogin(){
    ...
    const activateTab=target=>{window.GJUAuthOtpUI?.backToAuth();document.querySelectorAll("[data-auth-tab]").forEach(b=>b.classList.toggle("is-active",b.dataset.authTab===target));loginForm?.classList.toggle("hidden",target!=="login");signupForm?.classList.toggle("hidden",target!=="signup");show("");};
    ...
    loginForm?.addEventListener("submit",async e=>{
      e.preventDefault();busy(loginForm,true);show("");
      try{
        await authPersistenceReady;
        const email=$("#loginEmail").value.trim(),pass=$("#loginPassword").value,res=await signInWithEmailAndPassword(auth,email,pass);
        if(!res.user.emailVerified){await beginUnverifiedLogin(res.user,pass);return;}
        await completeVerifiedLogin(res.user,{},true);
      }catch(err){show(readableError(err),true);}finally{busy(loginForm,false);}
    });

    signupForm?.addEventListener("submit",async e=>{
      e.preventDefault();busy(signupForm,true);show("");
      try{
        await authPersistenceReady;
        ...
        const challenge=await requestSignupOtp(res.user,{mobile,display_name:name});
        pendingSignup={email,password:pass,name,mobile,challenge};clearSession();syncHeader(null);window.GJUAuthOtpUI?.showSignupOtp(email);show("Account created. Enter the OTP sent to your email to activate it.",false);
  ...
  function bindDashboard(){
    ...
    onAuthStateChanged(auth,user=>{
      authStateKnown=true;const trusted=isVerifiedUser(user);syncHeader(trusted?user:null);markAuthReady();
      if(!trusted){clearSession();if(logoutBtn)logoutBtn.textContent="Login";if(loading)loading.hidden=false;if(guest)guest.hidden=true;if(content)content.hidden=true;return;}
      rememberSession(user);if(logoutBtn)logoutBtn.textContent="Logout";if(loading)loading.hidden=true;if(guest)guest.hidden=true;if(content)content.hidden=false;
      if($("#userName"))$("#userName").textContent=user.displayName||"GovJobUpdates User";if($("#userEmail"))$("#userEmail").textContent=user.email||"No email available";if($("#profileName"))$("#profileName").textContent=user.displayName||"GovJobUpdates User";if($("#profileEmail"))$("#profileEmail").textContent=user.email||"No email available";syncMySqlAccount(user);
    });
  ...
  function readableError(err){const code=String(err?.code||"");const msg=String(err?.message||"");if(code.includes("invalid-credential"))return"Email ya password galat hai.";if(code.includes("email-already-in-use"))return"Is email se account already bana hua hai. Login karke OTP verify karein.";if(code.includes("weak-password"))return"Password kam se kam 6 characters ka rakho.";if(code.includes("too-many-requests")||code.includes("rate_limited"))return"Too many requests. Please wait and try again.";if(code.includes("invalid_otp"))return"OTP galat hai. Please check and try again.";if(code.includes("otp_expired"))return"OTP expire ho gaya hai. Please resend OTP.";if(code.includes("unauthorized-domain"))return"Firebase me govjobupdates.com ko Authorized domains me add karo.";if(code.includes("popup"))return"Google popup complete nahi hua. Dobara try karo.";if(msg&&msg!=="request_failed")return msg;return"Request failed. Please try again.";}
