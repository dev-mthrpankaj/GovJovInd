...
(function(){
  ...
  function syncHeader(user){const link=document.querySelector("[data-auth-entry], .header-login-btn");if(!link)return;const label=link.querySelector("span")||link;if(user){link.href=link.dataset.dashboardHref||headerHref("dashboard.html");link.classList.add("is-active");link.setAttribute("aria-label","Open candidate dashboard");label.textContent="Dashboard";}else{link.href=link.dataset.loginHref||headerHref("login.html");link.classList.remove("is-active");link.setAttribute("aria-label","Login to candidate dashboard");label.textContent="Login";}}
