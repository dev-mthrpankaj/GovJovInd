(function () {
  ...
  function applyRegisteredPassagePools(target) {
    const bank = Array.isArray(window.GJU_TYPING_PASSAGE_BANK) ? window.GJU_TYPING_PASSAGE_BANK : [];

    bank.forEach((set) => {
      if (!set || !set.language || !set.category || !set.exam || !set.levels) {
        return;
      }

      target[set.language] = target[set.language] || {};
      target[set.language][set.category] = target[set.language][set.category] || {};
      target[set.language][set.category][set.exam] = target[set.language][set.category][set.exam] || {};

      ["easy", "medium", "hard"].forEach((level) => {
        const passages = normalizePassages(set.levels[level]);
        if (passages.length) {
          target[set.language][set.category][set.exam][level] = passages;
        }
      });
    });
  }

  function normalizePassages(value) {
